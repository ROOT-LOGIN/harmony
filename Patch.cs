...
#pragma warning restore XS0001
    ...
    /// Patch数据
    /// </summary>
    public struct PatchInfoData
    {
        internal byte[] orgbytes;
        internal byte[] jmpbytes;
        internal long orgaddress;
        internal long jmpaddress;
        ...
        public void patch()
        {
            ILCopying.Memory.PROTECTION old;
            ILCopying.Memory.VirtualProtect(new IntPtr(orgaddress), jmpbytes.Length, ILCopying.Memory.PROTECTION.PAGE_EXECUTE_READWRITE, out old);
            ILCopying.Memory.WriteBytes(orgaddress, jmpbytes);
            ILCopying.Memory.VirtualProtect(new IntPtr(orgaddress), jmpbytes.Length, old, out old);
        }
        ...
        {
            ILCopying.Memory.PROTECTION old;
            ILCopying.Memory.VirtualProtect(new IntPtr(orgaddress), orgbytes.Length, ILCopying.Memory.PROTECTION.PAGE_EXECUTE_READWRITE, out old);
            ILCopying.Memory.WriteBytes(orgaddress, orgbytes);
            ILCopying.Memory.VirtualProtect(new IntPtr(orgaddress), orgbytes.Length, old, out old);
        }
    }
	...
	public class PatchInfo
	{
        ...
        [NonSerialized]
        internal PatchInfoData patchdata;
		...
		public void AddPrefix(MethodInfo patch, int ownerid, int priority, string[] before, string[] after)
		{
			var l = prefixes.ToList();
			l.Add(new Patch(patch, prefixes.Count() + 1, ownerid, priority, before, after));
			prefixes = l.ToArray();
		}

		public void AddPostfix(MethodInfo patch, int ownerid, int priority, string[] before, string[] after)
		{
			var l = postfixes.ToList();
			l.Add(new Patch(patch, postfixes.Count() + 1, ownerid, priority, before, after));
			postfixes = l.ToArray();
		}

		public void AddTranspiler(MethodInfo patch, int ownerid, int priority, string[] before, string[] after)
		{
			var l = transpilers.ToList();
			l.Add(new Patch(patch, transpilers.Count() + 1, ownerid, priority, before, after));
	...
	public class Patch : IComparable
	{
		...
		public Patch(MethodInfo patch, int index, int ownerid, int priority, string[] before, string[] after)
		{
