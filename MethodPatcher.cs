...
namespace Harmony
{
	...
	{
        /// <summary>
        /// 直接使源方法跳转到目标方法.（可以用于P/Invoke的注入或普通方法的直接跳转）
        /// </summary>
        /// <param name="source">源.</param>
        /// <param name="destination">目标.</param>
        /// <returns></returns>
        public static PatchInfoData Patch(MethodInfo source, MethodInfo destination)
        {
            var info = new PatchInfoData() {
                orgaddress = Memory.GetMethodStart(source),
                jmpaddress = Memory.GetMethodStart(destination)
            };

            Memory.WriteJump(info.orgaddress, info.jmpaddress, out info.orgbytes, out info.jmpbytes);
            return info;
        }

		// special parameter names that can be used in prefix and postfix methods
		//
		public static string INSTANCE_PARAM = "this";
...
#if ALLOW_ATTRIBUTES
            customParam = GetParameterOverride(method.GetParameterAttributes(), name);
        ...
        static void EmitCallParameter(ILGenerator il, MethodBase original, MethodInfo patch, Dictionary<string, LocalBuilder> variables)
		{
            ...
            if(isInstance && patchParameters.Length == originalParameters.Length)
            {
                // emit this pointer for instance patch    
        ...
        static bool AddPostfixes(ILGenerator il, MethodBase original, List<MethodInfo> postfixes, Dictionary<string, LocalBuilder> variables)
        {
            /*if(resultVariable != null && postfixes.Count == 1)
            {
                var fix = postfixes[0];
            ...
            for (int i = 0; i < postfixes.Count; i++)
            {
                ...
                */
                ...
                if(i == postfixes.Count - 1)
                {
                    ret = fix.ReturnType != typeof(void);
                }
            }
